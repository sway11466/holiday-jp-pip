...
from holiday_jp import Holiday, HolidayJP, JST


def test_extends_appears_in_all() -> None:
    custom = Holiday(year=2099, month=12, date=31, name="未来", local_date=datetime(2099, 12, 31, tzinfo=JST))
    holidayjp = HolidayJP(extends=[custom])
    assert custom in holidayjp.all()


def test_extends_into_existing_year() -> None:
    custom = Holiday(year=2024, month=2, date=29, name="閏年特別", local_date=datetime(2024, 2, 29, tzinfo=JST))
    holidayjp = HolidayJP(extends=[custom])
...
def test_extends_into_new_year() -> None:
    custom = Holiday(year=2099, month=1, date=1, name="未来元日", local_date=datetime(2099, 1, 1, tzinfo=JST))
    holidayjp = HolidayJP(extends=[custom])
