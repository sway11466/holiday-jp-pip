...
| `local_date` | 実行環境 TZ の `Date`（瞬間） | `datetime`（`tzinfo=JST`、時刻 0:00） | JS と等価。`local_date.astimezone()` で実行環境 TZ に再解釈可能（v0.2.0 で `date` から変更、[ADR](doc/architecture.md#local_date-は-jst-000-を表す-tz-付き-datetime-とする) 参照） |
