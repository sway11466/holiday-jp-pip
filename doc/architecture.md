...
## 外部 CSV 指定はバンドル CSV を置換する（マージしない）
   - Decision（決定事項）
      - `HolidayJP(csv_path=...)` で外部 CSV を指定した場合、バンドル CSV は **使わない**（置換）
      - バンドル CSV に独自レコードを追加したい場合は既存の `extends` を使う
   - Context（経緯・背景情報）
      - 「内閣府 CSV を更新版に差し替えたい」「テスト用に最小データを使いたい」「独自の祝日カレンダーを丸ごと使いたい」など、置換ニーズが主
      - マージしたいケースは `extends` で十分カバーできるため、API の入口を 2 つに増やす必要がない
      - 同一パスでも毎回ファイルを読む（インスタンス化は稀。キャッシュは必要になってから検討）
   - Consideration（比較・検討内容）
      - バンドル CSV と外部 CSV をマージする
         - バンドルにある期間と外部で重複したときの優先順位や重複排除のルールを決める必要があり、API が複雑化する
         - 「どのレコードがどこから来たか」が利用者から見えづらくなる
      - チルダ `~` や環境変数を自動展開する
         - 利用者環境の前提が増えると挙動の予測が難しくなる。`pathlib.Path.expanduser()` を呼ぶのは利用側責務とする
