...
公開は **GitHub Actions + Trusted Publishing（OIDC）** で行う。長期 API トークンは保持しない。
...
GitHub Actions / Trusted Publishing が使えない緊急時のみ利用する。通常は上記の自動公開を使うこと。

#### 1. API トークンを発行

1. PyPI / TestPyPI でアカウントを作成（未作成の場合）
...
```bash
# pyproject.toml と holiday_jp/__init__.py の __version__ をバンプしてからタグを切る
